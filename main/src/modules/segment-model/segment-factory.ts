...
import { 
  Segment, TextSegment, WeightedSegment, 
  PresetSegment, InlineWildcardSegment,
  BracketType, PresetMode
} from './types';
 ...
 * @returns 계산된 가중치 값
 */
export const calculateDisplayValue = memoize((bracketLevel: number, bracketType: BracketType): number => {
  // 중립 가중치 (레벨 0)
...
export function createTextSegment(content: string): TextSegment {
  if (content === undefined) {
    throw new Error('Content is required for text segment');
  }
  
  return {
    id: generateSegmentId('txt'),
    type: 'text',
    content: content || '' // 빈 문자열로 기본값 설정
  };
}
...
export function createPresetSegment(
  ...
  if (mode === 'fixed' && !selected) {
    console.warn(`Selected value should be provided for fixed preset "${name}"`);
  }
 ...
 */
export function createInlineWildcardSegment(options: string[]): InlineWildcardSegment {
  // 매개변수 검증
