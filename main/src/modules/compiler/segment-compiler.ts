...
import { CompileOptions, NovelAIPrompt, WildcardExpansionOptions } from './types';
 ...
 * @returns 컴파일된 텍스트
 */
export function compileSegmentTree(rootSegment: Segment, options: CompileOptions = {}): string {
...
function processSegment(
  ...
  if (isPresetSegment(segment)) {
    const result = compilePresetSegment(segment, context.random, context.expandWildcards);
  ...
  if (isWeightedSegment(segment)) {
    ...
    const childrenContent = segment.children
      ?.map(child => processSegment(child, context))
      .join('') || '';
    
    // 가중치 적용
    const result = applyBrackets(childrenContent, segment.bracketType, segment.bracketLevel);
    // console.log(`Weighted segment result: "${result}"`);
    return result;
  }
  
  if (isInlineWildcardSegment(segment)) {
    const result = compileInlineWildcard(segment, context.random, context.expandWildcards);
    // console.log(`Inline wildcard result: "${result}"`);
    return result;
  }
  
  // 타입별 처리가 없는 경우, 자식 세그먼트 처리
  const unknownSegment = segment as unknown as { children?: Segment[], type?: string };
  if (unknownSegment.children && unknownSegment.children.length > 0) {
    ...
    const results = unknownSegment.children.map((child: Segment) => processSegment(child, context));
 ...
 * @returns 괄호로 감싸진 텍스트
 */
function applyBrackets(content: string, bracketType: BracketType, bracketLevel: number): string {
  // 중립 가중치는 그대로 반환
  if (bracketLevel === 0) {
    return content;
  }
 ...
 * @returns 컴파일된 텍스트
 */
function compileInlineWildcard(
...
export function expandNestedWildcards(
  ...
  // 재귀적으로 와일드카드 확장하는 내부 함수
  function expand(innerText: string, depth: number): string {
    // 최대 중첩 깊이 제한 (무한 순환 방지)
    if (depth > maxDepth) {
      return `[최대 중첩 깊이 초과: ${innerText}]`;
    }
    
    // 인라인 와일드카드 패턴 검색
    return innerText.replace(/\(([^()]*(?:\([^()]*\)[^()]*)*)\)/g, (_match, options) => {
      ...
      return expand(selected, depth + 1);
    });
  }
...
export function compileToNovelAIPrompt(
  ...
  if (rootSegments.main.positive && segments[rootSegments.main.positive]) {
    result.mainPositive = compileSegmentTree(segments[rootSegments.main.positive], options);
  }
  
  if (rootSegments.main.negative && segments[rootSegments.main.negative]) {
    result.mainNegative = compileSegmentTree(segments[rootSegments.main.negative], options);
  }
  ...
  for (const [indexStr, character] of Object.entries(rootSegments.characters)) {
    ...
    if (character.positive && segments[character.positive]) {
      result.characters[index].positive = compileSegmentTree(segments[character.positive], options);
    }
    
    if (character.negative && segments[character.negative]) {
      result.characters[index].negative = compileSegmentTree(segments[character.negative], options);
    }
  }
...
export async function compileSegmentTreeAsync(
  ...
  if (typeof Worker === 'undefined') {
    console.warn('Web Workers not supported. Falling back to synchronous compilation.');
    return compileSegmentTree(rootSegment, options);
  }
