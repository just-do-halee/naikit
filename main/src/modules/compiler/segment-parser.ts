...
function compileSegmentToText(segment: Segment): string {
  if (segment.type === 'text') {
    ...
    const content = segment.children?.map(c => compileSegmentToText(c)).join('') || '';
 ...
 */
function countConsecutiveBrackets(text: string, startPos: number): [number, number] {
...
export function parseNovelAIPrompt(text: string): Segment {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    // 가중치 종료 감지
      ...
      // 연속된 닫는 괄호 개수 세기
      const [closingBracketCount, lastIndex] = countConsecutiveBrackets(text, i);
      ...
      if (stack.length > 1) {
        ...
        if (!isValid || !levelMatches) {
          ...
          // 내용 추가
          errorText += segment.children?.map(child => compileSegmentToText(child)).join('') || '';
    ...
    // 느낌표 시작 (프리셋 가능성)
      ...
      // 프리셋 이름 추출 시도
      const presetMatch = text.slice(i).match(/^!([a-zA-Z0-9_\-\u3131-\uD79D가-힣]+)/);
    ...
    // 키워드 패턴 인식
    else if (currentText === "" && /[a-zA-Z0-9_\-\u3131-\uD79D가-힣]/.test(char)) {
      // 키워드 패턴 검사
      const keywordMatch = text.slice(i).match(/^([a-zA-Z0-9_\-\u3131-\uD79D가-힣]+):([^,\s:]+)/);
      ...
      if (keywordMatch) {
        ...
        // 키워드 세그먼트 생성
        const keywordSegment = createPresetSegment(keywordName, 'fixed', keywordValue);
  ...
  while (stack.length > 1) {
    ...
    // 내용 추가
    unclosedText += segment.children?.map(child => compileSegmentToText(child)).join('') || '';
 ...
 */
function findMatchingClosingBracket(text: string, openPos: number): [number, number] {
